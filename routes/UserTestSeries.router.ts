...
router.post('/enroll', isAuth, authorizeAdmin, UserTestSeriesController.enrollInTestSeries);

// Get all test series for a specific user
router.get('/user/:userId/testseries', isAuth, authorizeAdmin, UserTestSeriesController.getUserTestSeries);

// Unenroll a user from a test series
router.delete('/unenroll', isAuth, authorizeAdmin, UserTestSeriesController.unenrollFromTestSeries);
